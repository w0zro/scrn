...
func tmuxConf(scrn string, scrollback, navWidth int) string {
	...
	w("# Written by scrn at every launch; edits do not survive one.",
		...
		"",
		"# The mouse is tmux's, so that it reaches every pane: a click goes to",
		"# the pane under it and takes the keys there, a program that speaks",
		"# mouse gets its own clicks and wheel, and in one that does not, the",
		"# wheel scrolls the transcript and a drag or a double click selects",
		"# and copies. These are tmux's own bindings, put back after the unbind.",
		"set -g mouse on",
		`bind -T root MouseDown1Pane select-pane -t = \; send-keys -M`,
		`bind -T root MouseDrag1Pane if -F "#{||:#{pane_in_mode},#{mouse_any_flag}}" "send-keys -M" "copy-mode -M"`,
		`bind -T root WheelUpPane if -F "#{||:#{pane_in_mode},#{mouse_any_flag}}" "send-keys -M" "copy-mode -e"`,
		`bind -T root DoubleClick1Pane select-pane -t = \; if -F "#{||:#{pane_in_mode},#{mouse_any_flag}}" "send-keys -M" "copy-mode -H ; send-keys -X select-word ; run-shell -d 0.3 ; send-keys -X copy-pipe-and-cancel"`,
		`bind -T root TripleClick1Pane select-pane -t = \; if -F "#{||:#{pane_in_mode},#{mouse_any_flag}}" "send-keys -M" "copy-mode -H ; send-keys -X select-line ; run-shell -d 0.3 ; send-keys -X copy-pipe-and-cancel"`,
		"",
